	...
	<table id="list" class="common_table3" width="99%" border="0"
		...
		<tr class="table_foot1">
			<td colspan="14" style="text-align:right"><input type="button"
				class="button-blue button-left" id="unjoinButton"  value="取消加盟" />
				<div class="result_foot">
					<span>当前&nbsp;${page.pageNo }&nbsp;/&nbsp;${page.totalPages
						}&nbsp;页</span> 
					<c:if test="${page.hasPre}">
						<span><a
							href="<%=hotelPath %>getHotelCustomerInfoRight.do?page.start=0&page.size=${page.pageSize}">首页</a>
						</span>
						<span><a
							href="<%=hotelPath %>getHotelCustomerInfoRight.do?page.start=${page.preFirst}&page.size=${page.pageSize}">上一页</a>
						</span>
					</c:if>
					<c:if test="${page.hasNext}">
						<span><a
							href="<%=hotelPath %>getHotelCustomerInfoRight.do?page.start=${page.nextFirst}&page.size=${page.pageSize}">下一页</a>
						</span>
						<span><a
							href="<%=hotelPath %>getHotelCustomerInfoRight.do?page.start=${page.lastFirst }&page.size=${page.pageSize}"">末页</a>
						</span>
					</c:if>
	...
	$(function(){
		 $("#list tr.contentTr").click(function(){
			 ...
			 var cbk =tr.children("td").children(":checkbox");
				 ...
				 var url = "<%=hotelPath %>getHotelCustomerInfoLeft.do";
				 url+="?guid=";
				 url+=id;
				parent.attr("src",url);
		 ...
		 if(checkboxList.length<=0)
		 {
		  alert("请选择酒店！");
			...
			if(confirm("确实取消?")){
						$.ajax({
			  			url:"<%=hotelPath %>updateUnstatus.do",
			  			type: "POST",
			  			dataType:"json",
 						...
 						/* 	var ckbs=$("input[name=checkbox]:checked");
									...
									返回第一页
									**/
									    var url = "<%=hotelPath %>getHotelCustomerInfoRight.do?page.size=${page.pageSize}";
					    	  			 url+="&page.pageNo="+"1";
					    	   			 window.location=url;	
					    	   		/**
									清空左边页
