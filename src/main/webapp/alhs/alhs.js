...
//计算2个日期相差天数
function calcDays(date1,date2,targetId){
 var date1 = document.getElementById(date1).value;
 var date2 = document.getElementById(date2).value;
 if(date1 && date2){
   var days = DateDiff(date1.substring(0,10),date2.substring(0,10));
   document.getElementById(targetId).value=days;
  }
}
//获取指定日期的前后日期（n可正可负，表示相差天数）
function addDate(date,n){
    //y:year m:month d:date  n可正可负
	this.dateAdd=function(y,m,d,n)
	{
    ...
    var reg = /^(\d{4})(\d{2})(\d{2})/;
	if(reg.test(date.replace(/-/g,"")))
	{
	    var d = this.dateAdd(RegExp.$1,RegExp.$2,RegExp.$3,n);
...
function checkedSelect(pWindow){
       if(pWindow){
          parentWindow = pWindow;
       }
       
       if(/first/.test(location)){
                  parentWindow.arry = null;
        }
		if(parentWindow.arry){
			var chks = document.getElementsByName("chks");
			for(i = 0 ; i < chks.length ; i++){
				var chk = chks[i];
				for( var x =  0 ; x < parentWindow.arry.length ; x++){
					if(chk.value === parentWindow.arry[x]){
...
function checkall(a){
 ...
 for(var i=0;i<chks.length;i++){
  chks[i].checked=a.checked; 
        addValue(chks[i]);
   }
}

function addValue(chk){
	if(!parentWindow.arry){parentWindow.arry= [];}
	var arry = [];
	for( i = 0 ; i < parentWindow.arry.length ; i++){
		arry.push(parentWindow.arry[i]);
	}
	//先移除
	for( i = 0 ; i < parentWindow.length ; i++){
			if(arry[i] === chk.value){
			   arry.splice(i,1);
               break;
			}
	...
	if(chk.checked){
	   arry.push(chk.value);
        ...
        parentWindow.arry = arry;
}

function addEvent(toObj,eventName,fn){   
    try{
        var f = (typeof(fn) == "function")?fn:function(){eval(fn)}; 
    if(window.attachEvent){
        toObj.attachEvent("on"+eventName,f);
    }else{
        toObj.addEventListener(eventName,f,false);
    }
    }catch(e){
       alert(e);
    }
...
function createDiv(e,div){
    var o = document.getElementById(div).style;   //菜单
    if(o.display =="none")
    {
        ...
        if (document.body.clientHeight + dt - t - h >= ch) o.top = ((p=="textarea" || p=="input")? t + h : t + h + 6) + "px";
        else o.top  = ((t - dt < ch) ? ((p=="textarea" || p=="input")? t + h : t + h + 6) : t - ch) + "px";
        if (dw + dl - l >= cw) o.left = l + "px"; else o.left = ((dw >= cw) ? dw - cw + dl : dl)+ "px";
        o.left = (parseInt(o.left)) + "px";
    }
    else{
       o.display="none";
    }
...
//自动创建div，按菜单定位
function createDiv1(divId,iframeId,divWidth,divHeight){  
    var e = __getEvent();
...
//自动创建div，页面中间
function createDiv2(divId,iframeId,divWidth,divHeight){  
    var s = document.getElementById(divId);
    //创建div,iframe
    if(!s){
	    var div = document.createElement("div");
    ...
    {
        o.display = "";
        o.top = ((parseInt(document.body.clientHeight))-(parseInt(document.body.scrollTop))-(parseInt(o.height)))/2 + "px";
        o.left = ((parseInt(document.body.clientWidth))-(parseInt(document.body.scrollLeft))-(parseInt(o.width)))/2 + "px";
...
//列的对齐方式（带统计）
function _adjust1(index, position)
{  
        var tab = document.getElementById("_listTab");

        for(var i=1; tab != null && i<tab.rows.length-2; i++)
        {
            var _row = tab.rows[i];
            var img = _row.getElementsByTagName("IMG");
            if(img.length == 1 && img[0].src.indexOf("page_but.gif") != -1) continue;
            if(index < _row.cells.length && _row.cells[index].className != "listTd") _row.cells[index].style.textAlign = position;
        }
    
}

//隐藏显示指定列
function hiddenRows(tabId,rowsArry,hiddenOrShow){
       var tab = document.getElementById(tabId);
       for(var i =0; tab != null && i<tab.rows.length-1;i++){
          for(var j=0; rowsArry!=null &&j<rowsArry.length;j++){
              var rowNo = rowsArry[j];
              tab.rows[i].cells[rowNo].style.display=hiddenOrShow?"none":"";
          }
       }
}
...
function getValueByXml(XmlId){
  var obj = "";
  var str = document.getElementById(XmlId);
  var xmlTopNode = __loadXML(str.getAttribute("src"));
  var els = xmlTopNode.getElementsByTagName("Element");
    for(var i=0; i<els.length; i++)
    {
        strValue = els[i].firstChild.nodeValue;
        var chk = "<input type=checkbox name=cks value='"+strValue+"'>"+strValue+"&nbsp;";
        if((i+1)%2==0){chk+="<br>"};
        obj += chk
    }
    return obj;
...
(function(){
 Chy = {  
	post : function(url,fun,method,tb){
		var xmlhttp;    
		try{
...
var yearMothInit=function(year,month,defaultYear,defaultMonth){
     var cmbYear = document.getElementById(year);
     var cmbMonth = document.getElementById(month);
     function addYear(cmbYear){
         var y = new Date().getFullYear();
         for(var i=y-5;i<=y+10;i++){
             var option = document.createElement("OPTION");
             cmbYear.options.add(option);
             option.value=i;
             option.innerHTML=i;
         }
     }
     function addMonth(cmbMonth){
         for(var i=1;i<=12;i++){
             var option = document.createElement("OPTION");
             cmbMonth.options.add(option);
             option.value=i<10?("0"+i):i;
             option.innerHTML=i<10?("0"+i):i;
         }
     }
     function defalueSelect(defaultYear,defaultMonth){
        for(var i=0; i<cmbYear.options.length; i++)
		{
			if(cmbYear.options[i].value == defaultYear)
			{
		...
		{  
			if(cmbMonth.options[i].value == defaultMonth)
			{
     ...
     defalueSelect(defaultYear,defaultMonth);
} 

function addTimeSelect(_cmb1,_cmb2,default1,default2,maxHours){
    var h = document.getElementById(_cmb1);
	var m = document.getElementById(_cmb2);

	for(var i=0;i<=(maxHours?maxHours:23);i++){
	   var option = document.createElement("OPTION");
	   var _i = (i<10?("0"+i):i);
	   h.options.add(option);
	   option.value = _i;
	   option.innerHTML = _i;
	   option.selected = (_i==default1)?"selected":"";
	...
	for(var j=0;j<60;j++){
	   ...
	   option.value = _j;
	   option.innerHTML = _j;
	   option.selected = (_j==default2)?"selected":"";
	   
	}
}

// 下拉框2级联动
var selectInit = function(_cmb1,_cmb2,default1,default2,arrayList)
{
	var cmb1 = document.getElementById(_cmb1);
	var cmb2 = document.getElementById(_cmb2);
	
	cmb1.options.length = 0;
	...
	function changeSelect()
	{
		...
		var item = cmb1.options[cmb1.selectedIndex].obj;
		
		for(var i=0; i<item.list.length; i++)
		{
			cmbAddOption(cmb2, item.list[i], item.list[i]);
		}
		
		cmbSelect(cmb2, default2);
	}
	
	for(var i=0; i<arrayList.length; i++)
	{
		cmbAddOption(cmb1, arrayList[i].name, arrayList[i]);
	}
	cmbSelect(cmb1, default1);
	changeSelect();
	cmb1.onchange = changeSelect;
}
